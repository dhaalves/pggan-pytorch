    ...
    def resl_scheduler(self):
        ...
        if self.fadein['gen'] is not None:
            if self.resl % 1.0 < self.trns_tick * delta:
                ...
                self.phase = 'gtrns'
            elif self.trns_tick * delta <= self.resl % 1.0 < (
                    self.trns_tick + self.stab_tick) * delta:
                self.phase = 'gstab'
        if self.fadein['dis'] is not None:
            if (self.trns_tick + self.stab_tick) * delta <= self.resl % 1.0 < (
    ...
    def train(self):
        ...
        for step in range(2, self.max_resl + 1 + 5):
            for iter in tqdm(range(0, (self.trns_tick * 2 + self.stab_tick * 2) * self.TICK, self.loader.batchsize)):
                ...
                # save model.
                self.snapshot(os.path.join(config.output_dir, 'model'))

                # save image grid.
                if self.globalIter % self.config.save_img_every == 0:
                    with torch.no_grad():
                        x_test = self.G(self.z_test)
                    utils.save_image_grid(x_test.data,
                                          os.path.join(config.output_dir, 'save/grid/{}_{}_G{}_D{}.jpg'.format(
                                              int(self.globalIter / self.config.save_img_every), self.phase,
                                              self.complete['gen'],
                                              self.complete['dis'])))
                    utils.save_image_single(x_test.data, os.path.join(config.output_dir,
                                                                      'save/resl_{}/{}_{}_G{}_D{}.jpg'.format(
                                                                          int(floor(self.resl)), int(
                                                                              self.globalIter / self.config.save_img_every),
                                                                          self.phase,
                                                                          self.complete['gen'],
                                                                          self.complete['dis'])))
